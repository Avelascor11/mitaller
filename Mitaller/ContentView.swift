    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let fixed = overview.fixedExpenses, fixed.pending > 0 {
                let fixedText = fixed.coverage?.headline ?? "Quedan \(formatMoney(fixed.pending, currency: overview.currency)) por separar este mes."
                let fixedDetail = fixed.coverage?.recommendation ?? "Sepáralos antes de hablar de beneficio real."
                HStack(spacing: 10) {
